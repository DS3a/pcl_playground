...
def filter_z(pcl_data, base=0, window = 5.5):
    np_array = np.asarray(pcl_data)
    o3d_pcd = o3d.geometry.PointCloud(
        o3d.utility.Vector3dVector(np_array)).voxel_down_sample(voxel_size=0.089) #`voxel_down_sample(voxel_size=0.05)` if it's too sluggish
